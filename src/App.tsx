  ...
  useEffect(() => {
    if (sessionState === 'running') {
      timerRef.current = setInterval(() => {
        ...
        // --- Health guard (configurable in Settings): N min focus without a real break → nudge ---
        if (settings.healthEnabled !== false) {
          ...
          if (focusSinceBreakRef.current >= focusLimit) {
            ...
            playCozyChime(false);
            setSpeechBubble(`${settings.healthFocusMinutes ?? 50} minutes of pure focus! Amazing... but let's stand up and stretch, okay? 🍊`);
            speakText(`You've been focusing for ${settings.healthFocusMinutes ?? 50} minutes! Time to stand up, stretch, and drink some water. Your health comes first!`, settings.speechEnabled);
          }
        }

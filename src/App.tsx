...
import { startRain, stopRain } from './utils/ambient';
import {
  Play,
  Pause,
  Square,
...
export default function App() {
  ...
  const pauseSession = () => setSessionState('paused');
  ...
  return (
    <div className="min-h-screen text-[#725442] flex flex-col font-sans relative">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-5 py-6">
        <AnimatePresence mode="wait">
          ...
          {currentPage === 'study' && (
            ...
            >
              {/* LEFT — time tracking (vertical) */}
              <div className="order-2 md:order-1 md:col-span-3 bg-white/90 backdrop-blur-md rounded-3xl border border-[#FFDEC9] p-5 shadow-sm flex flex-col items-center relative overflow-hidden">
                {/* Hero orange counter */}
                <div className="w-full flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      onClick={toggleRain}
                      title={rainOn ? 'Rain: on' : 'Ambient rain'}
                      className={`ml-1 p-2 rounded-xl border transition-all cursor-pointer active:scale-90 ${
                        rainOn
                          ? 'bg-[#9BB8C9] border-[#9BB8C9] text-white'
                          : 'bg-white border-[#FFE4CF] text-[#B99C86] hover:text-[#725442]'
                      }`}
      ...
      {/* Tiny footer */}
      <footer className="py-4 text-center text-[10px] text-[#B99C86] font-medium">
        Study with {settings.mascotName} <span className="text-rose-300">♥</span> all camera frames stay on your device
      </footer>

      {/* Floating study-question chat */}

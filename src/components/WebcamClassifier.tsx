...
export default function WebcamClassifier({
  ...
  // CRITICAL: Always remove stale scripts first to avoid version conflicts
  // that cause "t is not a function" runtime errors when switching models.
  const loadDependencies = (type: 'image' | 'pose'): Promise<void> => {
    return new Promise((resolve, reject) => {
      // Remove any previously injected TF / TM scripts to avoid version conflicts
      document.querySelectorAll('script[data-tm-script]').forEach((el) => el.remove());
      // Clear globals so stale references don't linger
